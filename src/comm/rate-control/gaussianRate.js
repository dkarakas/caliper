 ...
 */
class GaussianRateController extends RateInterface{
    /**
     * Creates a new instance of the {GaussianRateController} class.
     * @constructor
    ...
    async init(msg) {
        ...
        this.totalTxn = Number(this.options.total) / msg.clients;
    ...
    async applyRateControl(start, idx, recentResults) {
        let currentSleepTime = this._interpolate(start, this.duration, idx) / this.totalTxn;
    ...
    _interpolateFromIndex(start, duration, idx) {
        throw new Error('I do not have the CDF of a gaussian distribution');
    }
    ...
    _interpolateFromTime(start, duration, idx) {
        let sigma = duration / 6;
        let x = Date.now() - start - duration/2
        return Math.sqrt(2*Math.PI) * sigma * Math.exp(x*x/2/sigma/sigma);
    }
}

module.exports = GaussianRateController;